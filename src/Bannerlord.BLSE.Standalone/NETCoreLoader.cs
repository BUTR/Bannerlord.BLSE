...
public static class NETCoreLoader
{
    ...
    public static void Launch(string[] args)
    {
        ...
        var files1 = Directory.GetFiles(Path.Combine(path), "*.dll", SearchOption.TopDirectoryOnly)
            ...
            .Select(x => $"{x};");
        var files2 = Directory.GetFiles(Path.Combine(path, "Microsoft.NETCore.App"), "*.dll", SearchOption.TopDirectoryOnly).Select(x => $"{x};");
        var files3 = Directory.GetFiles(Path.Combine(path, "Microsoft.AspNetCore.App"), "*.dll", SearchOption.TopDirectoryOnly).Select(x => $"{x};");
        var files4 = Directory.GetFiles(Path.Combine(path, "Microsoft.WindowsDesktop.App"), "*.dll", SearchOption.TopDirectoryOnly).Select(x => $"{x};");
        // Do not set Modules .dll as trusted and to be loaded
        //var files5 = Directory.GetFiles(Path.Combine(path, "..", "..", "Modules"), "*.dll", SearchOption.AllDirectories).Select(x => $"{x};");
        // The instantly loaded assembly files
        var files = string.Join("", files1.Concat(files2).Concat(files3).Concat(files4)/*.Concat(files5)*/);
        ...
        var createDelegateResult = coreclr_create_delegate(
            pCLRRuntimeHost,
            (uint) domainId,
            NativeUTF8("Bannerlord.BLSE.Shared.NET"),
